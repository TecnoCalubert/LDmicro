    ...
    for(i = begin; i <= end; i++) {
        ...
        switch(IntCode[i].op) {
            ...
            case INT_DELAY:
                ...
                doIndent(f, i);
                fprintf(f, "  delay_us(%d);\n", IntCode[i].literal);
                doIndent(f, i);
                fprintf(f, "#elif defined(HI_TECH_C)\n");
                doIndent(f, i);
                fprintf(f, "  __delay_us(%d);\n", IntCode[i].literal);
                doIndent(f, i);
...
*/
                ...
                if((isVarInited(IntCode[i].name1) < 0) || 1 || (isVarInited(IntCode[i].name1) == rungNow)) {
        ...
        if(Prog.cycleTime > 0) {
            ...
            if(compile_MNU == MNU_COMPILE_CCS_PIC_C) {
                ...
                if(Prog.cycleTimer == 0) {
                    fprintf(f, "    setup_timer_0(T0_INTERNAL | T0_DIV_%d);\n", plcTmr.prescaler);
                } else {
                    fprintf(f, "    setup_timer_1(T1_INTERNAL | T1_DIV_BY_%d);\n", plcTmr.prescaler);
                }
            } else if(compile_MNU == MNU_COMPILE_HI_TECH_C) {
                if(Prog.cycleTimer == 0) {
                    fprintf(f,
