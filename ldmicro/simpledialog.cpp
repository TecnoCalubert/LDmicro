...
{
    return ShowSimpleDialog(title, boxes, labels, numOnlyMask, alnumOnlyMask, fixedFontMask, boxes, dests, 0, NULL);
}
...
{
    return ShowSimpleDialog(title, labs, labels, numOnlyMask, alnumOnlyMask, fixedFontMask, boxes, dests, 0, NULL);
}
...
void ShowSleepDialog(int which, SDWORD *delay, char *name)
{
    ...
    if(ShowSimpleDialog(s, 1, labels, (1 << 1), (1 << 0), (1 << 0), dests)) {
        ...
        if(del <= 0) {
            ...
            Error("%s\n\r%s %s\r\n%s", s1, s4, s2, s3);
        } else if((period >= (long long)(1 << (SizeOfVar(name) * 8 - 1))) && (Prog.mcu->portPrefix != 'L')) {
            const char *s1 =
